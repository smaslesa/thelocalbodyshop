...
export default function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Logo appears after scrolling past hero section (smoother transition at 350px)
  ...
  return (
    <>
      {/* Split Navigation with Massive Logo */}
      <motion.header 
      ...
      >
        <nav className={`px-6 md:px-12 lg:px-20 transition-all duration-500 ${
          ...
          <div className="max-w-[1600px] mx-auto">
            {/* Desktop Layout - Clean Left Logo Design */}
                      ...
                      <Link href="/" className="relative group">
                        <img
                          ...
                          className={`w-auto transition-all duration-500 object-contain ${
                            isScrolled 
                              ? 'h-14 brightness-100' 
                              : 'h-16 brightness-0 invert drop-shadow-[0_2px_8px_rgba(255,255,255,0.5)]'
                          }`}
                        />
                      </Link>
                    </motion.div>
              ...
              <div className="flex items-center gap-8">
                ...
                <a
                  ...
                  className={`flex items-center gap-2 text-[13px] font-semibold transition-all duration-300 whitespace-nowrap ${
                    isScrolled ? 'text-black hover:text-blue-600' : 'text-white hover:text-white/80'
                  }`}
                >
                ...
                <Link
                  href="/estimate"
                  className={`px-5 py-2.5 text-xs font-semibold uppercase tracking-wider transition-all duration-300 whitespace-nowrap ${
                    isScrolled 
                    ...
                    <Link href="/">
                      <img
                        ...
                        className={`w-auto transition-all duration-500 object-contain ${
                          isScrolled 
                            ? 'h-10 brightness-100' 
                            : 'h-12 brightness-0 invert drop-shadow-[0_2px_4px_rgba(255,255,255,0.3)]'
                        }`}
                      />
                    </Link>
                  </motion.div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <div className="w-6 h-5 flex flex-col justify-between">
                    <motion.span 
                      className={`w-full h-[2px] ${isScrolled ? 'bg-black' : 'bg-white'}`}
                      animate={{ 
                        rotate: isOpen ? 45 : 0,
                        y: isOpen ? 8 : 0
                      }}
      ...
      {/* Subtle Progress Bar */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-black to-transparent z-[60] origin-left"
            ...
            <div className="flex flex-col h-full">
                ...
                <div className="space-y-4">
                  {[...navItemsLeft, ...navItemsRight].map((item) => (
                    <Link
                      ...
                      href={item.href}
                      onClick={() => setIsOpen(false)}
                      className="block text-lg font-medium text-gray-900 hover:text-black transition-colors py-2"
                    >
                      {item.name}
                    </Link>
                  ))}
              ...
              <div className="px-6 py-6 space-y-4 border-t">
                ...
                <Link
                  href="/estimate"
                  onClick={() => setIsOpen(false)}
                  className="block w-full bg-black text-white text-center py-3 font-semibold hover:bg-gray-900 transition-colors"
                >
                  FREE CONSULTATION
                </Link>
              </div>
